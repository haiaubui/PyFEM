...
# -*- coding: utf-8 -*-
"""
Created on Wed Jun 27 17:02:11 2018
...
#        rie *= -1.0
        ...
        self.RiLambd[inod][0] -= rie[0]
        self.RiLambd[inod][1] -= rie[1]
...
#        pass
    def calculateK(self, K, inod, jnod, t):
        ...
        condt = np.array([self.condt[0],0.0,self.condt[1]],dtype = self.dtype)
        ...
        # n.F^-1 B.dC/du.B
        ktx = 0.5*self.multiplyCderiv(condt,condt,jnod)
        kt1212 += np.einsum('i,ij,k',self.normv,self.Fmgx,ktx)
    ...
    def calculateR(self, R, inod, t):
        ...
        ri12 -= nt
        ri12 /= self.JF*self.material.mu00
...
#        en = 0.5*np.dot(condt,condt)
#        ri12 = en*self.normv
#        ri12 -= np.dot(self.normv,self.condt)*self.condt
#        ri12 /= self.material.mu00
#        ri12 *= self.N_[inod]*self.getFactor()
        
        R[0] += t*ri12[0]
        R[1] += t*ri12[1]
        self.RiLambd[inod][0] += ri12[0]
        self.RiLambd[inod][1] += ri12[1]
        
#    def calculateRe(self, R, inod, t):
#        condt = np.array(self.condt)
#        rie = -condt*self.N_[inod]
#        rie *= self.getFactor()
#        R[0] = rie[0]
#        R[1] = rie[1]
#        condt = np.array(self.condt)
...
#        if np.fabs(e[1].getX()[1]-H) < 1.0e-13:    
#            condtx = condt
#            elementBs.append(AxiSymMagMech(e,2,QE.LagrangeBasis1D,\
#            QE.generateQuadNodeOrder(2,1),intDatB,normBndVec[i],i,condtx))
#            elementBs[-1].setMaterial(mat1)
