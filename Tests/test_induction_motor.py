...
import Element.PolarElement as PE
...
numberTimeSteps = 1200
rho_inf = 0.8
tol = 1.0e-8
load = 3.1e6*math.sqrt(2)
#load = 2045.175/((5.2e-2**2-3.2e-2**2)*np.pi/8.0)*math.sqrt(2.0)
...
def loadfuncB(x,t):
    return load*math.cos(60.0*2*np.pi*t-2.0*np.pi/3.0)
#    return 0.0
...
def loadfuncC(x,t):
    return load*math.cos(60.0*2*np.pi*t-4.0*np.pi/3.0)
#    return 0.0
...
def create_mesh():
    ...
    elements,belm = polarMesh(r0,r1,0.0,2.0*np.pi,50,1,Ndof,tOrder,typ='quadring')
    ...
    elements,belm = polarMesh(r1,r2,0.0,2.0*np.pi,50,2,Ndof,tOrder,typ='quadring',bnd=2)
...
#                    n.setLoad(loadfunc[math.ceil(i/2)],0)
        ...
        if belm is not None:
            belms = [PE.PolarElementGapBoundary(e,2,basisf,1.0/(np.pi*4.0e-7),nHarmonic=numH,rRatio=r2/r3,normv=[-1.0,0.0]) for e in belm]
            for e in belms:
                e.current = False
            mesh.addGapElements(belms)
...
output1 = FO.StandardFileOutput('/home/haiau/Documents/result_.dat')
output2 = FO.HarmonicsOutput('/home/haiau/Documents/result_harmonics_.dat')
output = [output1,output2]
#alg = NM.NonlinearNewmarkAlgorithm(mesh,tOrder,output,sv.numpySolver(),\
#totalTime, numberTimeSteps,rho_inf,tol=1.0e-8)

alg = NM.LinearNewmarkAlgorithm(mesh,tOrder,output,sv.numpySolver(),\
...
tout2,testout2 = output2.readOutput('/home/haiau/Documents/result_harmonics_.dat',list(range(0,numberTimeSteps,4)))

torque = []
for t in testout2:
    torque.append(0.0)
    for i in range(mesh.nHarmonic):
        j = i+1
        torque[-1] += -j**2*(mesh.rRatio**j)*(t[i][0]*t[i][3]-t[i][2]*t[i][1])
    torque[-1] *= 2.0*np.pi/(4.0e-7*np.pi)
...
for i in list(range(numberTimeSteps)):
    if i%(numberTimeSteps/200) > 0:
        continue
    times.append(i*alg.deltaT)
    output1.updateToMesh(mesh,i)
    res1 = 0.0
    for e in windingA1:
    ...
    voltage.append((res1-res2)*3.1e6/2045.175/math.sqrt(2.0))

testKtL = alg.KtL        

#output1.updateToMesh(mesh,30)
